...
import HybridSimulation.Robot.TYPE;
...
public class World extends CRBinWorld {
	...
	public void onTrialStart(int trialNumber_, int runNumber_) {
		...
		for (int d=0; d< settings.NumberofLeaderRobots; d++) {
			double xPos = CRMaths.getRandomInteger((int)size.x - 250, (int)size.x - 200);
			double yPos = CRMaths.getRandomInteger((int)size.y - 250, (int)size.y - 200);
