...
public class Robot extends BaseWorldObject{
	...
	protected boolean canDrop, closetoBoundary, farfromBoundary, placePheromone, returntoNest;
	protected int startedPushingTimeCounter, updateVal, pher, distancetoPosition, pheromoneSizeRE, LeadingPheromoneRE, pheromoneID;
	protected boolean goingtoFoodLocation, goingtoNestAlone, goingtoNestwithTrail, justEntered;
	protected boolean goingtoNest, goingtoRecruit, leadingPheromonePlaced, toCreateTrail, leadingReinforcementPlaced;
	protected boolean isRandomlySearching, returntoCollectFood, canPursue, clearSeenPheromones, readytoForage;
	...
	protected boolean newPheromoneFound, searchingFoodLocation, inNestwithFood, isFoodRemaining;
	CRVector3d newlocationtoMoveTo;
	ArrayList<Pheromones> newPheromone;
	protected ArrayList<BaseWorldObject> robotToPursue;
	protected ArrayList<Robot> robotCluster;
	protected double resistanceMultiplier;
	protected double feltResistance, newrotate;
	boolean turnLeft, turnRight, distanceReduced;
	protected boolean shouldRecordCarryTime;
	CRBaseSituatedModel obstacleCollidedWith;
	
	//initialise variables for the agent.
	public Robot(int id, CRVector3d pos, double rotation, TYPE Type) {
		...
		farfromBoundary = false;
		foodLocationFound = false;
		clearSeenPheromones = false;
		newPheromoneFound = false;
		justEntered = false;
		searchingFoodLocation = false;
		inNestwithFood = false;
		placePheromone = false;
		toCreateTrail = false;
		leadingReinforcementPlaced = false;
		returnFromExhaustedLocation = true;
		returntoNest = false;
		readytoForage = false;
		isFoodRemaining = true;
		turnLeft = turnRight = distanceReduced = false;
		obstacleCollidedWith = null;
		pher = 0;
		newrotate = 0;
		pheromoneID = 0;
		pheromoneCheck = 0;
		distancetoPosition = 0;
		newlocationtoMoveTo = new CRVector3d(position);
		...
		robotCluster = new ArrayList<Robot>();
	...
	public void onUpdateLoopStart() {
		...
		//check whether agent is in food location
		...
		//check whether agent is within pheromone trail
		Pheromones pheromon = null;
		...
		if (robotCluster.size()==0) {
			robotCluster.add(this);
		}
		...
		//get edge positions of agent's body
		...
		//determine what pheromone concentrations can be sensed from those edge positions
		...
		//If agent is a leader, run code below
		if (type == TYPE.LEADER) {
			//after food is found, go to nest to recruit
			if (foodLocationFound == true && goingtoRecruit == true && goingtoNest == true ) {
				...
				if ((presentPheromones.get(0).getPheromoneConcentrationAt(position) <= 0.3)) {
				...
				if ((presentPheromones.get(0).getPheromoneConcentrationAt(position) > 0.1) && leadingPheromonePlaced == false) {
					ArrayList<CRVector3d> pheromonePosition = new ArrayList<>();
					ArrayList<CRVector3d> pheromoneSize = new ArrayList<>();
			...
			//go to the edge of the food marker pheromone to create trail to nest.
			...
			//if is within nest's active space, drop food and restart foraging process
			...
			//return to continue forage when food has been dropped in the nest
			//move randomly until recruitment pheromomone is detected
			if (returntoCollectFood == true && isWithinFoodLocation == false) {
				...
				robotCluster = getRobotPosInCluster(this);
				boolean thereisLeader = false;
				robotCluster = getRobotPosInCluster(this);
				for (Robot rb : robotCluster) {
					for (Pheromones ptrail : pheromoneTrails) {
						if (ptrail.getPheromoneConcentrationAt(rb.getPosition())> 0.7) {
							thereisLeader = true;
						}
					}
				}
				if (presentPheromones.size() > 1) {
					for (int i=1; i<presentPheromones.size(); i++) {
						if (presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0 && thereisLeader == false && presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0.7 && presentPheromones.get(i).getsizer() > 43) {
							...
							pheromoneCheck = (int) presentPheromones.get(i).getsizer();
							pheromoneID = presentPheromones.get(i).getPhID();
						}
						if (presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0 && thereisLeader == true && presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0.6 && presentPheromones.get(i).getsizer() > 43) {
							rotationChange = (int) Math.toDegrees(this.getRelativeVectorTo(presentPheromones.get(i).getPosition()).x);
							this.setRotation(rotationChange);
							this.setThrustForce(minThrustForce);
							goingtoFoodLocation = true;
			...
			//At recruitment pheromone.
			//form cluster/flock based on the calculated opinion of needed flock mates.
			if (goingtoFoodLocation == true && searchingFoodLocation == false && goingtoNest == false) {
				leadingPheromonePlaced = false;
				double otherMaxPheromone = 0;
				double numberofRobots = 0;
				double thresholdVal = 0;
				if (readytoForage == false) {
					robotCluster = getRobotPosInCluster(this);
				}
				Boolean isInitiator = false;
				ArrayList <Robot>robots = ((World)world).getRobots();
				for (Robot r : robots) {
					if (presentPheromones.get(0).getPheromoneConcentrationAt(r.getPosition())>0 && this != r) {
						numberofRobots++;
					}
				}
				if (pheromoneCheck >= 50) {
					thresholdVal = numberofRobots/3;
				}
				else if (pheromoneCheck >= 47 && pheromoneCheck < 50) {
					thresholdVal = numberofRobots/6;
				}
				else if (pheromoneCheck < 47) {
					thresholdVal = numberofRobots/12;
				}
				//System.out.println("pheromoneCheck value : "+pheromoneCheck);
				if (readytoForage == false && robotCluster.size() > thresholdVal && isWithinNest == true) {
					robotCluster = getRobotPosInCluster(this);
					readytoForage = true;
				}
				//compare pheromone concentrations to decide who is initiator.
				if (readytoForage == true) {
					for (Robot v : robotCluster) {
						for(Pheromones pt : pheromoneTrails) {
							if(pt.getPheromoneConcentrationAt(v.getPosition())>otherMaxPheromone) {
								otherMaxPheromone = pt.getPheromoneConcentrationAt(v.getPosition());
							}
						}
					}
					
					if (maxTrailPheromone > otherMaxPheromone) {
						isInitiator = true;
					...
					if(isInitiator == false) {
						...
						//Use tropotaxis - edge positions of body to make movements 
						if (isWithinFoodLocation == false) {
							if (currentConc > 0.5) {
							...
							for (Pheromones phr : FoodLocations) {
								if (phr.getPheromoneConcentrationAt(position) > 0.1) {
			...
			//if in food location, start seaching for food.
			if (isWithinFoodLocation == true && searchingFoodLocation == true && goingtoNest == false) {
				robotCluster = getRobotPosInClusterToNest(this);
				...
				for (Robot rb : robotCluster) {
					for (Pheromones ptrail : pheromoneTrails) {
						if (ptrail.getPheromoneConcentrationAt(rb.getPosition())> 0.7) {
							thereisLeader = true;
							leaderRot = rb.getRotation();
						}
					}
				}
				//keep sensing to detect the releaser pheromone.
				for (Pheromones trail : pheromoneTrails) {
					if (trail.getPheromoneConcentrationAt(position)>0) {
						if (thereisLeader == false) {
							if (trail.getPheromoneConcentrationAt(position)> 0.7 && trail.getPheromoneConcentrationAt(position) > concentrate) {
								concentrate = trail.getPheromoneConcentrationAt(position);
							}
						}
						else {
							if (trail.getPheromoneConcentrationAt(position)> 0.6 && trail.getPheromoneConcentrationAt(position) > concentrate) {
								concentrate = trail.getPheromoneConcentrationAt(position);
							}
						}
					}
				...
				if (pushedObjects.size()>1) {
					if (goingtoNestwithTrail == true){
						if ((concentrate > 0.6 && thereisLeader == true) || (concentrate > 0.7 && thereisLeader == false)) {
							goingtoNest = true;
							robotLocation.copyFrom(position);
							this.setThrustForce(minThrustForce);
							this.setRotation(leaderRot);
						}
						else {
							goingtoNest = false;
						}
				...
				for (Pheromones t : pheromoneTrails) {
					if (t.getPheromoneConcentrationAt(position)>0) {
						if (robotCluster.size()>5) {
							this.setThrustForce(maxThrustForce);
							moveAwayFromDroppedObject(null, true);
						}
					}
			...
			if (goingtoNest == true && goingtoNestwithTrail == true && inNestwithFood == false) {
				...
				double otherMaxPheromone = 0;
				double numberofRobots = 0;
				double numberofFoods = 0;
				double totalNumberofFood = 0;
				double thresholdVal = 0;
				if (readytoForage == false) {
					robotCluster = getRobotPosInClusterToNest(this);
				}
				Boolean isInitiator = false;
				ArrayList <Robot>robots = ((World)world).getRobots();
				ArrayList <Foods> food = ((World)world).getFoods();
				totalNumberofFood = food.size();
				for (Robot r : robots) {
					for (Pheromones foodp : FoodLocations) {
						if (foodp.getPheromoneConcentrationAt(position) > 0 && foodp.getPheromoneConcentrationAt(r.getPosition())>0 && this != r) {
							numberofRobots++;
						}
					}
				}
				for (Foods f : food) {
					for (Pheromones fp : FoodLocations) {
						if (fp.getPheromoneConcentrationAt(position)>0 && fp.getPheromoneConcentrationAt(f.getPos())>0) {
							BaseWorldObject fd = (BaseWorldObject) f;
							//if (fd.getIsBeingPushed() == false) {
								numberofFoods++;
							//}
						}
					}
				...
				if (numberofFoods>0 && (numberofFoods/totalNumberofFood) > 0 && (numberofFoods/totalNumberofFood) < 0.125) {
					thresholdVal = numberofRobots;
				}
				else if (numberofFoods>0 && (numberofFoods/totalNumberofFood) > 0.125 && (numberofFoods/totalNumberofFood) < 0.25) {
					thresholdVal = numberofRobots/2;
				}
				else if (numberofFoods>0 && (numberofFoods/totalNumberofFood) > 0.25 && (numberofFoods/totalNumberofFood) < 0.5) {
					thresholdVal = numberofRobots/4;
				}
				else if (numberofFoods>0 && (numberofFoods/totalNumberofFood) > 0.5) {
					thresholdVal = numberofRobots/6;
				}
				//System.out.println("Nest approaching robotPos array size : "+thresholdVal);
				//System.out.println("food remaining in nest : "+numberofFoods/totalNumberofFood);
				if (readytoForage == false && robotCluster.size() > thresholdVal && isWithinFoodLocation == true) {
					readytoForage = true;
					robotCluster = getRobotPosInClusterToNest(this);
				}
				//System.out.println("Nest approaching readytoForage status : "+readytoForage);
				//compare pheromone concentrations to decide who is initiator.
				if (readytoForage == true) {
					for (Robot v : robotCluster) {
						for(Pheromones pt : pheromoneTrails) {
					...
					if(isInitiator == false) {
						//reinforce pheromones if it has decayed.
						reinforcePheromones();
						...
						//reinforce trail when food has been foraged and pheromone is decaying
						reinforcePheromones();
					}
					if (isWithinNest == true) {
						rotationChange = -(int) Math.toDegrees(this.getRelativeVectorTo(nestLocation).x);
						//System.out.println("relative degree to nest from home location: "+rotationChange );
						if (Math.random() < WALK_RANDOMISATION_ANGLE_CHANGE_PROBABILITY) {
							rotationChange += CRMaths.getRandomInteger(WALK_RANDOMISATION_MIN_ANGLE_CHANGE, WALK_RANDOMISATION__MAX_ANGLE_CHANGE, true);
			...
			if (goingtoNest == true && goingtoNestAlone == true && inNestwithFood == false) {
				...
				if (isWithinNest == false) {
					...
					if ((presentPheromones.get(0).getPheromoneConcentrationAt(position) <= 0.1) && leadingPheromonePlaced == false && placePheromone == true) {
						if ((robotLocation.x - this.getPosition().x >= 5) || (robotLocation.y - this.getPosition().y >= 5) || (robotLocation.x - this.getPosition().x <= -5) || (robotLocation.y - this.getPosition().y <= -5)) {
							ArrayList<CRVector3d> pheromonePosition = new ArrayList<>();
							ArrayList<CRVector3d> pheromoneSize = new ArrayList<>();
							pheromonePosition.add(this.getPosition());
		...
		if (type == TYPE.FOLLOWER) {
			...
			if(foodLocationFound == true && goingtoFoodLocation == false && isWithinNest == true) {
				//System.out.println("can pursue : "+canPursue);
				//System.out.println("seenPheromone size : "+seenPheromoneIDs.size());
				isWaitingtoFollow = true;
				goingtoFoodLocation = true;
			}
			
			
			//move randomly in the nest and sense for releaser pheromones
				...
				if (presentPheromones.size() > 1) {
					for (int i=1; i<presentPheromones.size(); i++) {
						if (presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0 && thereisLeader == false && presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0.7 && presentPheromones.get(i).getsizer() > 43) {
							...
							pheromoneCheck = (int) presentPheromones.get(i).getsizer();
							pheromoneID = presentPheromones.get(i).getPhID();
							//System.out.println("size of leading pheromone : "+presentPheromones.get(i).getsizer());
						}
						if (presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0 && thereisLeader == true && presentPheromones.get(i).getPheromoneConcentrationAt(position) > 0.6 && presentPheromones.get(i).getsizer() > 43) {
							this.setRotation(leaderRot);
							this.setThrustForce(minThrustForce);
			...
			//form cluster in the recruitment pheromone
			//make flock size opinion, check if is initiator
			//go to food location
			if (isWaitingtoFollow == true && searchingFoodLocation == false) {
				double otherMaxPheromone = 0;
				double numberofRobots = 0;
				double thresholdVal = 0;
				Pheromones pheromone = null;
				boolean isInitiator = false;
				if (readytoForage == false) {
					robotCluster = getRobotPosInCluster(this);
				}
				...
				//System.out.println("threshold value : "+thresholdVal);
				//System.out.println("pheromoneCheck value : "+numberofRobots);
				//System.out.println("robotPos array size : "+robotCluster.size());
				if (readytoForage == false && robotCluster.size() > thresholdVal && isWithinNest == true) {
					readytoForage = true;
					robotCluster = getRobotPosInCluster(this);
				}
				//System.out.println("readytoForage status : "+readytoForage);
				//compare pheromone concentrations to decide who is initiator.
				if (readytoForage == true) {
			...
			//going to nest with trail, form cluster and go to nest
			if (goingtoNest == true && goingtoNestwithTrail == true && inNestwithFood == false) {
				searchingFoodLocation = false;
				double otherMaxPheromone = 0;
				double numberofRobots = 0;
		...
		//if food has exhausted, or unsuccessfully traverse the pheromone trail, - go back to the nest.
		if (isWithinFoodLocation == true || isWithinNest == false || (isWithinNest == true && (goingtoFoodLocation == true || isWaitingtoFollow == true))) {
			//check if food is present in location.
			//if not head straight to home
			if((searchingFoodLocation == true && goingtoNest == false) || goingtoNestwithTrail == true) {
				boolean foodremain = checkforFood(this);
				if (maxPheromoneCheck > 0) {
					if (maxPheromoneCheck < 44 || foodremain == false) {
						if(pushedObjects.size()>1) {
							returnFromExhaustedLocation = true;
							returntoNest = true;
							searchingFoodLocation = false;
			...
			if (isWithinFoodLocation == true) {
				isFoodRemaining = checkforFood(this);
				if (checkforFood(this) == false) {
					((World)world).timeofFoodExhaustion(((World)world).getController().getTimeCounterSinceTrialStart());
				}
			}
			if (isWithinNest == true) {
				for (Pheromones phr : presentPheromones) {
					if (phr.getsizer() > 25 && phr.getsizer() < 100) {
						if (phr.getPheromoneConcentrationAt(position)>0) {
							if (phr.getsizer() < 44) {
								returntoNest = true;
								resetDecisionVariables();
							}
				...
				for (Pheromones p : pheromoneTrails) {
					if (p.getsizer()>25) {
						if (p.getPheromoneConcentrationAt(position)>0) {
							Boolean isThereLeader = false;
							if (robotCluster.size()>1) {
								for (Robot b : robotCluster) {
									if (p.getPheromoneConcentrationAt(b.getPosition())>0.7) {
										isThereLeader = true;
										justEntered = true;
									}
								}
								if (p.getPheromoneConcentrationAt(position)>0.7) {
									isThereLeader = true;
									justEntered = true;
								}
								if ((isThereLeader == false && justEntered == false) || (readytoForage == true && robotCluster.size()<1)) {
									returntoNest = true;
									resetDecisionVariables();
								}
							}
				...
				if (isWithinFoodLocation == false && isWithinNest == false && isWithinTrail == false) {
					if (foodLocationFound == true) {
						if (goingtoNestAlone == true || goingtoNestwithTrail == true || goingtoFoodLocation == true) {
							boolean isAnyoneInPher = false;
							for(Robot r : robotCluster) {
								for (Pheromones p : pheromoneTrails) {
									if (p.getPheromoneConcentrationAt(r.getPosition())>0) {
										isAnyoneInPher = true;
									}
								}
							}
							if (isAnyoneInPher == false) {
								returntoNest = true;
								resetDecisionVariables();
							}
						}
		...
		//go back to the nest - active space.
		...
		if (isFoodRemaining == false) {
			for (Pheromones pm : pheromoneTrails) {
				if (pm.getsizer() > 25) {
					if (pm.getPheromoneConcentrationAt(position)>0 && pm.getPhID() == pheromoneID) {
						pm.setsizer(new CRVector3d(pm.getsizer() - 9, pm.getsizer() - 9, 0));
					}
				}
			}
		}
	...
	public void reinforcePheromones() {
		...
		if ((type == TYPE.LEADER && goingtoFoodLocation == false) || (type == TYPE.FOLLOWER && isWaitingtoFollow == false)) {
			if (leadingPheromoneReinforcement == true) {
				reinforcePheromoneLead.setsizer(new CRVector3d(50, 50, 0));
			}
		}
	}
	//check for phermone conc before flocking
	//if conc is highest, let the rest flock around you
	public void FlockForage() {
		CRVector3d vec1 = new CRVector3d(0,0,0);
		CRVector3d vec2 = new CRVector3d(0,0,0);
		CRVector3d velocity = new CRVector3d(0,0,0);
		double thrust = 0;
		double rotation = 0;
		ArrayList<Pheromones>presentPheromones = ((World)world).getPheromones();
		rotation = alignmentRule();
		thrust = thrustAlignment();
		reinforcePheromones();
		this.setThrustForce(thrust);
		this.setRotation(rotation);
	}
	
	
	public Double alignmentRule() {
		double maxp = 0;
		double rotation = 0;
		Robot rob = null;
		ArrayList <Double> rot = new ArrayList<>();
		rot = this.getRobotRotationInCluster(this, robotCluster);
		for (Double d : rot) {
			rotation = rotation + d;
		}
		rotation = rotation/rot.size();
		ArrayList<Pheromones>presentPheromones = ((World)world).getPheromones();
		for (Pheromones p : presentPheromones){
			if (p.getsizer()<=50) {
				for (Robot r : robotCluster) {
					if (p.getPheromoneConcentrationAt(r.getPosition())>maxp) {
						maxp = p.getPheromoneConcentrationAt(r.getPosition());
						rob = r;
					}
				}
			}
		...
		if (maxp == 0 || maxp<0) {
			...
			return rob.getRotation();
		}
	}
	
	public Double thrustAlignment() {
		double thrust = 0;
		double maxp = 0;
		Robot rob = null;
		ArrayList<Pheromones>presentPheromones = ((World)world).getPheromones();
		...
		if (maxp == 0 || maxp<0) {
			for (Robot rb : robotCluster) {
				if (rb.getThrustForce()>maxp) {
					maxp = rb.getThrustForce();
				}
			}
			thrust = maxp;
		}
		else {
			thrust = rob.getThrustForce();
		}
		
		return thrust;
	}
	//set color of agent based on its kind
	...
	//reset variables to continue foraging processes.
	...
	public CRVector3d getPositionr() {
		return this.getPosition();
	}
	
	public double getThrustForcer() {
		return this.getThrustForce();
	}
	
	public boolean checkifPushingFood() {
		if (this.pushedObjects.size()>1) {
			return true;
		}
		else {
			return false;
	...
	public ArrayList<Robot> getCluster(){
		return robotCluster;
	}
